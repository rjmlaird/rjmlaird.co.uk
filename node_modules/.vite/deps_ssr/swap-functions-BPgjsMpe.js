import "./astro_runtime_server_render_server-islands-shared__js.js";
//#region node_modules/astro/dist/transitions/swap-functions.js
...
var knownVueScopedStyles = /* @__PURE__ */ new Map();
...
function deselectScripts(doc) {
	for (const s2 of doc.scripts) if (!s2.hasAttribute("data-astro-rerun") && detectScriptExecuted(s2)) s2.dataset.astroExec = "";
}
function swapRootAttributes(newDoc) {
	const currentRoot = document.documentElement;
	const nonOverridableAstroAttributes = [...currentRoot.attributes].filter(({ name }) => (currentRoot.removeAttribute(name), NON_OVERRIDABLE_ASTRO_ATTRS.includes(name)));
	[...newDoc.documentElement.attributes, ...nonOverridableAstroAttributes].forEach(({ name, value }) => currentRoot.setAttribute(name, value));
}
function swapHeadElements(doc) {
	const relevantNodes = (parent, what = "both") => [...[...parent.childNodes].filter((child) => child.nodeType === 8 && child.textContent === "[if astro]>server-island-start<![endif]" || what === "both" && child.nodeType === 1)];
	...
	if (import.meta.env.DEV) relevantNodes(doc.head).forEach((child) => {
		document.head.append(knownVueScopedStyles.get(child.dataset?.viteDevId) || child);
	});
	else document.head.append(...relevantNodes(doc.head));
}
function swapBodyElement(newElement, oldElement) {
	...
	for (const el of oldElement.querySelectorAll(`[${PERSIST_ATTR}]`)) {
		...
		if (moveBefore) moveBefore(docEl, el, null);
		else docEl.appendChild(el);
	}
...
var saveFocus = () => {
	...
	} else return () => restoreFocus({ activeElement: null });
};
...
var persistedHeadElement = (el, newDoc) => {
	...
	if (import.meta.env.DEV && el instanceof HTMLStyleElement) {
		const viteDevId = vueScopedStyleId(el);
		if (viteDevId) return newDoc.head.querySelector(`style[data-vite-dev-id="${viteDevId}"]`);
	}
	if (el.tagName === "STYLE" && el.textContent) {
		const styles = newDoc.head.querySelectorAll("style");
		for (const s of styles) if (s.textContent === el.textContent) return s;
	}
...
//#endregion
export { swap as a, swapHeadElements as c, saveFocus as i, swapRootAttributes as l, detectScriptExecuted as n, swapBodyElement as o, restoreFocus as r, swapFunctions as s, deselectScripts as t, vueScopedStyleId as u };
