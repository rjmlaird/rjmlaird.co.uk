...
function parse(g, C = "@") {
	if (!E) return init.then(() => parse(g));
	const I = g.length + 1, w = (E.__heap_base.value || E.__heap_base) + 4 * I - E.memory.buffer.byteLength;
	w > 0 && E.memory.grow(Math.ceil(w / 65536));
	const D = E.sa(I - 1);
	if ((A ? B : Q)(g, new Uint16Array(E.memory.buffer, D, I)), !E.parse()) throw Object.assign(/* @__PURE__ */ new Error(`Parse error ${C}:${g.slice(0, E.e()).split("\n").length}:${E.e() - g.lastIndexOf("\n", E.e() - 1)}`), { idx: E.e() });
	const o = [], i = [];
	for (; E.ri();) {
		const A = E.is(), Q = E.ie(), B = E.it(), C = E.ai(), I = E.id(), w = E.ss(), D = E.se();
		let i;
		E.ip() && (i = G(g.slice(-1 === I ? A - 1 : A, -1 === I ? Q + 1 : Q)));
		let k = null;
		for (k = [], E.rsa(); E.ra();) {
			const A = E.aks(), Q = E.ake(), B = E.avs(), C = E.ave();
			k.push([J(g.slice(A, Q)), J(g.slice(B, C))]);
		}
		0 === k.length && (k = null), o.push({
			n: i,
			t: B,
			s: A,
			e: Q,
	...
	for (; E.re();) {
		const A = E.es(), Q = E.ee(), B = E.els(), C = E.ele(), I = B < 0 ? void 0 : J(g.slice(B, C)), w = J(g.slice(A, Q));
		i.push({
			s: A,
			e: Q,
			ls: B,
...
var init = WebAssembly.compile(g()).then(WebAssembly.instantiate).then(({ exports: A }) => {
	E = A;
});
var initSync = () => {
	if (E) return;
	const A = new WebAssembly.Module(g());
	E = new WebAssembly.Instance(A).exports;
};
//#endregion
