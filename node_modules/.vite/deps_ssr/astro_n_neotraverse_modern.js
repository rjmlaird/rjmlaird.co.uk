//#region node_modules/neotraverse/dist/modern/modern.js
...
function copy(src, options) {
	if (typeof src === "object" && src !== null) {
		let dst;
		if (is_array(src)) dst = [];
		else if (is_date(src)) dst = new Date(src.getTime ? src.getTime() : src);
		else if (is_regexp(src)) dst = new RegExp(src);
		else if (is_error(src)) dst = { message: src.message };
		else if (is_boolean(src) || is_number(src) || is_string(src)) dst = Object(src);
		...
		for (const key of iterator_function(src)) dst[key] = src[key];
		return dst;
	}
	return src;
}
	...
	return function walker(node_) {
		...
		if (typeof state.node === "object" && state.node !== null && !state.circular) {
			...
			for (const [index, key] of Object.entries(state.keys ?? [])) {
				path.push(key);
				if (modifiers.pre) modifiers.pre(state, state.node[key], key);
				const child = walker(state.node[key]);
				if (immutable && has_own_property.call(state.node, key) && !is_writable(state.node, key)) state.node[key] = child.node;
				child.isLast = state.keys?.length ? +index === state.keys.length - 1 : false;
				child.isFirst = +index === 0;
				if (modifiers.post) modifiers.post(state, child);
				path.pop();
			}
			parents.pop();
		}
...
var Traverse = class {
	...
	*/
	get(paths) {
		...
		for (let i = 0; node && i < paths.length; i++) {
			const key = paths[i];
			if (!has_own_property.call(node, key) || !this.#options.includeSymbols && typeof key === "symbol") return;
			node = node[key];
		}
		return node;
	}
	...
	*/
	has(paths) {
		...
		for (let i = 0; node && i < paths.length; i++) {
			const key = paths[i];
			if (!has_own_property.call(node, key) || !this.#options.includeSymbols && typeof key === "symbol") return false;
			node = node[key];
		}
		return true;
	}
	...
	*/
	clone() {
		...
		if (is_typed_array(this.#value)) return this.#value.slice();
		return function clone(src) {
			...
			if (typeof src === "object" && src !== null) {
				...
				for (const key of iteratorFunction(src)) dst[key] = clone(src[key]);
				parents.pop();
				nodes.pop();
				return dst;
			}
