//#region node_modules/astro/dist/runtime/server/astro-island.js
var FORBIDDEN_COMPONENT_EXPORT_KEYS = /* @__PURE__ */ new Set([
	"__proto__",
	"constructor",
	"prototype"
]);
{
	...
	class AstroIsland extends HTMLElement {
		...
		handleHydrationError(error) {
			...
			if (this.dispatchEvent(event)) console.error(`[astro-island] Error hydrating ${componentUrl}`, error);
		}
		async start() {
			...
			try {
				await Astro[directive](async () => {
					const rendererUrl = this.getAttribute("renderer-url");
					try {
						const [componentModule, { default: hydrator }] = await Promise.all([this.importWithRetry(this.getAttribute("component-url")), rendererUrl ? this.importWithRetry(rendererUrl) : Promise.resolve({ default: () => () => {} })]);
						const componentExport = this.getAttribute("component-export") || "default";
						if (!componentExport.includes(".")) {
							if (FORBIDDEN_COMPONENT_EXPORT_KEYS.has(componentExport)) throw new Error(`Invalid component export path: ${componentExport}`);
							this.Component = componentModule[componentExport];
						} else {
							this.Component = componentModule;
							for (const part of componentExport.split(".")) {
								if (FORBIDDEN_COMPONENT_EXPORT_KEYS.has(part) || !this.Component || typeof this.Component !== "object" && typeof this.Component !== "function" || !Object.hasOwn(this.Component, part)) throw new Error(`Invalid component export path: ${componentExport}`);
		...
		hydrate = async () => {
			...
			try {
				...
				if (componentExport) componentName += ` (export ${componentExport})`;
				console.error(`[hydrate] Error parsing props for component ${componentName}`, this.getAttribute("props"), e);
				throw e;
			}
			...
			if (process.env.NODE_ENV === "development" && hydrationTimeStart) this.setAttribute("client-render-time", (performance.now() - hydrationTimeStart).toString());
	...
	if (!customElements.get("astro-island")) customElements.define("astro-island", AstroIsland);
}
//#endregion
export {};
