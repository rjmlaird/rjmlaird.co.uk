import { n as detectScriptExecuted } from "./swap-functions-BPgjsMpe.js";
import { a as onPageLoad, i as doSwap, o as triggerEvent, r as doPreparation, s as updateScrollPosition } from "./events-DXduTiSX.js";
import { internalFetchHeaders } from "virtual:astro:adapter-config/client";
//#region node_modules/astro/dist/transitions/router.js
...
async function fetchHTML(href, init) {
	try {
		...
		for (const [key, value] of Object.entries(internalFetchHeaders)) headers.set(key, value);
		const res = await fetch(href, {
			...init,
			headers
		});
		const mediaType = (res.headers.get("content-type") ?? "").split(";", 1)[0].trim();
...
function runScripts() {
	...
	for (const script of document.getElementsByTagName("script")) script.dataset.astroExec === void 0 && script.getAttribute("type") === "module" && (needsWaitForInlineModuleScript = script.getAttribute("src") === null);
	needsWaitForInlineModuleScript && document.body.insertAdjacentHTML("beforeend", `<script type="module" src="data:application/javascript,"/>`);
...
var moveToLocation = (to, from, options, pageTitleForBrowserHistory, historyState) => {
	...
	if (to.href !== location.href && !historyState) if (options.history === "replace") {
	...
	if (historyState) scrollTo(historyState.scrollX, historyState.scrollY);
	else {
		if (to.hash) {
			...
			if (!history.state) {
				...
				if (intraPage) window.dispatchEvent(new PopStateEvent("popstate"));
			}
		} else if (!scrolledToTop) scrollTo({
			left: 0,
			top: 0,
			behavior: "instant"
		});
...
function preloadStyleLinks(newDocument) {
	const links = [];
	for (const el of newDocument.querySelectorAll("head link[rel=stylesheet]")) if (!document.querySelector(`[${PERSIST_ATTR}="${el.getAttribute(PERSIST_ATTR)}"], link[rel=stylesheet][href="${el.getAttribute("href")}"]`)) {
...
async function updateDOM(preparationEvent, options, currentTransition, historyState, fallback) {
	async function animate(phase) {
		function isInfinite(animation) {
			...
			return window.getComputedStyle(effect.target, effect.pseudoElement).animationIterationCount === "infinite";
		}
	...
	const animateFallbackOld = async () => {
		if (fallback === "animate" && !currentTransition.transitionSkipped && !preparationEvent.signal.aborted) try {
			await animate("old");
		} catch {}
	};
	...
	if (fallback === "animate" && !currentTransition.transitionSkipped && !swapEvent.signal.aborted) animate("new").finally(() => currentTransition.viewTransitionFinished());
...
function abortAndRecreateMostRecentNavigation() {
	...
	return mostRecentNavigation = { controller: new AbortController() };
}
async function transition(direction, from, to, options, historyState, hasUAVisualTransition = false) {
	...
	if (navigationType !== "traverse") updateScrollPosition({
		scrollX,
		scrollY
	});
	...
	if (prepEvent.defaultPrevented || prepEvent.signal.aborted) {
		if (currentNavigation === mostRecentNavigation) mostRecentNavigation = void 0;
		if (!prepEvent.signal.aborted) location.href = to.href;
		return;
	}
	async function defaultLoader(preparationEvent) {
		...
		if (import.meta.env.DEV && !preparationEvent.signal.aborted) await prepareForClientOnlyComponents(preparationEvent.newDocument, preparationEvent.to, preparationEvent.signal);
	}
	...
	if (supportsViewTransitions && !hasUAVisualTransition) currentTransition.viewTransition = document.startViewTransition(async () => await updateDOM(prepEvent, options, currentTransition, historyState));
	else {
...
var onScrollEnd = () => {
	if (history.state && (scrollX !== history.state.scrollX || scrollY !== history.state.scrollY)) updateScrollPosition({
		scrollX,
		scrollY
	});
};
		...
		if ("onscrollend" in window) addEventListener("scrollend", onScrollEnd);
			...
			const scrollInterval = () => {
				...
				if (lastY === scrollY && lastX === scrollX) {
					...
					onScrollEnd();
					return;
				} else lastY = scrollY, lastX = scrollX;
			};
...
async function prepareForClientOnlyComponents(newDocument, toLocation, signal) {
	if (newDocument.body.querySelector(`astro-island[client='only']`)) {
		...
		if (nextHead) [...nextHead.querySelectorAll(`style[${VITE_ID}]`)].map((style) => style.getAttribute(VITE_ID)).forEach((id) => {
			const style = nextHead.querySelector(`style[${VITE_ID}="${id}"]`);
			if (style && !newDocument.head.querySelector(`style[${VITE_ID}="${id}"]`)) newDocument.head.appendChild(style.cloneNode(true));
		});
		async function hydrationDone(loadingPage) {
			if (!signal.aborted) await new Promise((r) => loadingPage.contentWindow?.addEventListener("load", r, { once: true }));
