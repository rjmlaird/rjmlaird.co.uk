...
import { i as __require, n as __esmMin, o as __toESM, t as __commonJSMin } from "./rolldown-runtime_BVj4LQFE.mjs";
...
var init_from = __esmMin(() => {
	...
	blobFrom = (path, type) => stat(path).then((stat) => fromBlob(stat, path, type));
	fileFrom = (path, type) => stat(path).then((stat) => fromFile(stat, path, type));
	fileFromSync = (path, type) => fromFile(statSync(path), path, type);
	fromBlob = (stat, path, type = "") => new Blob([new BlobDataItem({
		path,
		size: stat.size,
		lastModified: stat.mtimeMs,
		start: 0
	})], { type });
	fromFile = (stat, path, type = "") => new File([new BlobDataItem({
		path,
		size: stat.size,
		lastModified: stat.mtimeMs,
		start: 0
...
//#endregion
export { init_from as a, init_esm_min as c, Blob as d, init_fetch_blob as f, fileFromSync as i, File as l, blobFromSync as n, FormData as o, fileFrom as r, formDataToBlob as s, blobFrom as t, init_file as u };
